...
OUT_PATH = "dashboard_per_modality.pdf"
...
def heatmap(ax, pivot_metric, title, cmap="RdYlGn"):
    ...
    groups = ["json", "card"]
    mat2 = np.full((len(groups), len(datasets)), np.nan)
    mat2[0, :] = [(x+y)/2 for x, y in zip(mat[0], mat[1])]
    mat2[1, :] = [(x+y)/2 for x, y in zip(mat[2], mat[3])]
    im = ax.imshow(mat2, cmap=cmap, aspect="auto", vmin=0, vmax=1 if mat[~np.isnan(mat)].max() <= 1 else None)
...
with PdfPages(OUT_PATH) as pdf:
    ...
    # ── Row 1: Format comparison (JSON vs Card) ───────────────────────────────
    ax10 = fig.add_subplot(gs[0, 0])
    grouped_bar(ax10, format_groups, "coverage_score", PALETTE, "Coverage Score\n(JSON vs Card)", "Score", ylim=(0, 1))

    ax11 = fig.add_subplot(gs[0, 1])
    grouped_bar(ax11, format_groups, "hallucination_rate", PALETTE, "Hallucination Rate\n(JSON vs Card)", "Rate", ylim=(0, 1))

    ax12 = fig.add_subplot(gs[0, 2])
    grouped_bar(ax12, format_groups, "semantic_mean_sim", PALETTE, "Semantic Consistency\n(JSON vs Card)", "Cosine Sim", ylim=(0, 1))
    
    # ── Row 3: Radar + Heatmap ────────────────────────────────────────────────
    ax30 = fig.add_subplot(gs[1, 0], polar=True)
    ...
    ax30 = fig.add_subplot(gs[1, 1], polar=True)
    radar_cats = ["Coverage", "ROUGE-L F1", "Semantic Sim", "1-Hallucination", "BLEU"]
    radar_data = {}
    cols = []
    for grp in GROUP_COLORS:
    ...
    fig2.text(0.5, 0.975, "Card vs JSON — Quality Delta per Dataset",
    ...
    for idx, (metric, label) in enumerate(metrics_p2):
        ...
        # JSON bar (avg of yprov+json and flowcept+json)
        json_vals = [df[(df["format"] == "json") & (df["dataset"] == d)][metric].mean() for d in datasets]
        card_vals = [df[(df["format"] == "card") & (df["dataset"] == d)][metric].mean() for d in datasets]

        b1 = ax.bar(x - width / 2, json_vals, width, color=PALETTE["json"], label="JSON", zorder=3)
        b2 = ax.bar(x + width / 2, card_vals, width, color=PALETTE["card"], label="Card", zorder=3)
