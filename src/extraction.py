...
def detect_schema(source_path: str) -> str:
    ...
    if path.endswith("_f.jsonl") or path.endswith("_f.json"):
        return "flowcept", "json"
    elif path.endswith("_y.json") or path.endswith("_y.jsonl"):
        return "yprov", "json"
    elif path.endswith("_f.md"): 
        return "flowcept", "md"
    elif path.endswith("_y.md"): 
        return "yprov", "md"
    else: 
        raise Exception("detect_schema failed")
...
def _extract_flowcept_md(text: str) -> dict:
    """
    Extract FlowCept provenance attributes from a markdown document
    generated by _flowcept_to_reference().

    The expected sections are:
      ## Workflow Identity     → workflow_id, campaign_id, name, user, …
      ## Execution Environment → platform.*, cpu.*, memory.*, disk.*
      ## Tasks                 → total_tasks, activity_ids
      ## Process               → pid, executable, cmd_line, num_threads

    Returns a flat dict with the same keys as _extract_flowcept().
    """
    sections = _parse_md_sections(text)
    attrs: dict = {}

    # --- Workflow Identity ---
    for k, v in _kv_lines_to_dict(sections.get("Workflow Identity", [])).items():
        attrs[k] = v                           # workflow_id, campaign_id, name, …

    # --- Execution Environment ---
    # The renderer strips the "machine." prefix, so we restore it here.
    env = _kv_lines_to_dict(sections.get("Execution Environment", []))
    for k, v in env.items():
        attrs[f"machine.{k}"] = v              # machine.platform.system, machine.cpu.brand_raw, …

    # --- Tasks ---
    tasks = _kv_lines_to_dict(sections.get("Tasks", []))
    if "total_tasks" in tasks:
        attrs["total_tasks"] = tasks["total_tasks"]
    if "activity_ids" in tasks:
        attrs["activity_ids"] = tasks["activity_ids"]

    # --- Process ---
    for k, v in _kv_lines_to_dict(sections.get("Process", [])).items():
        attrs[f"process.{k}"] = v              # process.pid, process.executable, …

    # --- Preamble: source file line ---
    for line in sections.get("__preamble__", []):
        if line.startswith("Source file:"):
            attrs["source_file"] = line.split("Source file:", 1)[1].strip()
        elif line.startswith("Schema:"):
            attrs["schema"] = line.split("Schema:", 1)[1].strip()
...
def _extract_yprov_md(text: str) -> dict:
    """
    Extract yProv4ML provenance attributes from a markdown document
    generated by _yprov_to_reference().

    The expected sections are:
      ## Experiment Identity   → yprov:experiment_name, yprov:run_id, yprov:PID, …
      ## Timing                → prov:startedAtTime, prov:endedAtTime
      ## Workflow Steps        → steps (list of activity names)
      ## Dataset Provenance    → entity.* keys (files, modality, domain, …)
      ## Environment           → yprov:python_version, yprov:provenance_path, …

    Returns a flat dict with the same keys as _extract_yprov().
    """
    sections = _parse_md_sections(text)
    attrs: dict = {}
    ...
    # --- Experiment Identity ---
    # Keys keep their yprov: / prov: prefixes in the rendered output
    for k, v in _kv_lines_to_dict(sections.get("Experiment Identity", [])).items():
        if k == "agent":
            attrs["agent"] = v
        else:
            attrs[k] = v   # yprov:experiment_name, yprov:run_id, yprov:PID, …

    # --- Timing ---
    for k, v in _kv_lines_to_dict(sections.get("Timing", [])).items():
        attrs[k] = v       # prov:startedAtTime, prov:endedAtTime

    # --- Workflow Steps ---
    # Rendered as:  "- steps: ['Inspect', 'Infer', …]"
    steps_raw = _kv_lines_to_dict(sections.get("Workflow Steps", []))
    if "steps" in steps_raw:
        attrs["workflow_steps"] = steps_raw["steps"]

    # --- Dataset Provenance ---
    # The renderer strips the "entity." prefix, so we restore it.
    for k, v in _kv_lines_to_dict(sections.get("Dataset Provenance", [])).items():
        attrs[f"entity.{k}"] = v   # entity.files, entity.modality, entity.likely_domain, …

    # --- Environment ---
    for k, v in _kv_lines_to_dict(sections.get("Environment", [])).items():
        attrs[k] = v       # yprov:python_version, yprov:provenance_path, yprov:artifact_uri
...
def compute_reference_from_source(file: str) -> str:
    ...
    elif schema == "yprov" and filetype == "json":
        data  = json.load(open(source_path, "r"))
        attrs = _extract_yprov(data)
        return _yprov_to_reference(attrs, source_path)
    
    elif schema == "flowcept" and filetype == "md":
        records = _load_md(source_path)
        attrs = _extract_flowcept_md(records)
        return _flowcept_to_reference(attrs, source_path)
    
    elif schema == "yprov" and filetype == "md":
        records = _load_md(source_path)
        attrs = _extract_yprov_md(records)
        return _flowcept_to_reference(attrs, source_path)
...
def compute_ground_truth_attrs_from_source(file: str) -> dict:
    ...
    if schema == "flowcept" and filetype == "json" or filetype == "jsonl":
        records = _load_jsonl(source_path)
        return _extract_flowcept(records)
    elif schema == "yprov" and filetype == "json" or filetype == "jsonl":
        data = json.load(open(source_path))
        return _extract_yprov(data)
    
    elif schema == "flowcept" and filetype == "md":
        records = _load_md(source_path)
        return _extract_flowcept_md(records)
        # return _flowcept_to_reference(attrs, source_path)
    
    elif schema == "yprov" and filetype == "md":
        records = _load_md(source_path)
        return _extract_yprov_md(records)
        # return _flowcept_to_reference(attrs, source_path)
